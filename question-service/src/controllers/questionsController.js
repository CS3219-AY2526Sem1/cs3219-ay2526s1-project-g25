 ...
 * Required fields validated by middleware: title, description, difficulty, topic, test_cases, image_url.
 */
export async function createQuestion(req, res) {
  try {
    const { title, description, difficulty, topic, test_cases, image_url = null } = req.body;

    // Insert into DB. ID and created_at generated by DB default gen_random_uuid() and now().
    const payload = { title, description, difficulty, topic, test_cases, image_url };
...
export async function updateQuestion(req, res) {
  try {
    const { id } = req.params;
    const updates = {};
    const allowed = ['title','description','difficulty','topic','test_cases', 'image_url'];
    allowed.forEach(k => {
